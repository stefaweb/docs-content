...
8. Provide an SSL/TLS certificate for your subdomain so that it can serve traffic over HTTPS. You have three options for this:
...
4. In the **Endpoint** panel, click **Reset**. 
    ...
    A screen displays warning you that this will reset the bucket's domain back to the default Edge Services endpoint. Edge Services will consider your customized subdomain as unknown. You should also remember to delete your CNAME record from your domain provider, and any SSL/TLS certificates you imported into Secret Manager (if no longer required elsewhere, so that you are no longer billed for it). If you generated a managed Let's Encrypt certificate however, Scaleway takes care of the deletion for you.
...
You can view your Edge Services metrics via [Scaleway Cockpit](/observability/cockpit/quickstart/). This allows you to monitor your ingress, egress, request rate and cache hit ratio as well as other metrics. Access your Edge Services dashboard in the Scaleway console via the shortcut in Edge Services tab of the bucket in question.

### Understanding the dashboard

The Grafana dashboard presents a number of different metrics. Use the `Bucket name` dropdown in the top left to select which bucket to view Edge Services metrics for, and the time range dropdown in the top right to modify the time period to apply to the metrics.
...
- **Cache hit ratio**: The percentage of requests served from Edge Services's cache, compared to the total number of requests in total to Edge Services for this bucket, over the specified time period.
...
- **Edge Services response statuses**: A visual representation of the distribution of HTTP response statuses for requests served by Edge Services over the specified time period.
    - **200**: `HTTP 200 OK` response. Edge Services sent the requested resource is sent in full, indicating that it was being served for the first time, or had been been modified since the client's last request. 
    - **400**: `HTTP 304 Not Modified` response. Edge Services informed the client that the requested resource had not been modified since their last request, allowing the client to use their cached version rather than Edge Services resending the resource in full.

- **Request origin country**: The proportion of requests (to Edge Services for the specified bucket over the specified time period) originating from different geographical countries.
...
4. In the **Disable Edge Services** panel at the bottom of the screen, click **Disable Edge Services**.
    ...
    <Message type="tip">
    Remember to delete the CNAME record with your domain provider, and any SSL/TLS certificates for the custom domain that you imported into Secret Manager. If you generated a managed Let's Encrypt certificate, it will be automatically deleted with no further action required.
    </Message>
