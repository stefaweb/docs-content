...
This is possible with the introduction of our [VPC routing](/network/vpc/concepts/#routing) feature. Managed routes allow a VPC to route traffic between the private IP addresses of resources attached to different Private Networks within the same VPC. This feature is currently in public beta, and can only be [activated when you create a new VPC](/network/vpc/how-to/manage-routing/). We are planning to make it available on existing VPCs in the near future, along with other features such as custom routing.
